...
export default function Home() {
  ...
  const getBalanceSlotNames = () => {
    return [
      ...LINES.map((line) => teamSlots.team1[line].trim()),
      ...LINES.map((line) => teamSlots.team2[line].trim()),
    ];
  };

  const getAutoBalanceMembers = () => {
    const names = getBalanceSlotNames();

    if (names.some((name) => !name)) {
      return null;
    }

    const foundMembers = names.map((name) => findMemberByInput(name));

    if (foundMembers.some((member) => member === null)) {
      return null;
    }
    ...
    return foundMembers as Member[];
  };

  const canAutoBalanceTeams = getAutoBalanceMembers() !== null;

  const getCombinations = <T,>(items: T[], count: number) => {
    const result: T[][] = [];

    const pick = (startIndex: number, selected: T[]) => {
  ...
  const buildTeamSlotsFromMembers = (teamMembers: Member[]) => {
    const slots = {} as Record<LineKey, string>;

    LINES.forEach((line, index) => {
      slots[line] = teamMembers[index].name;
    });

    return slots;
  };

  const getTeamScoreByMembers = (teamMembers: Member[]) => {
    return LINES.reduce((total, line, index) => {
      return total + calculateMemberScore(teamMembers[index], line);
    }, 0);
  };
  ...
  const getLineDiffTotalByMembers = (team1Members: Member[], team2Members: Member[]) => {
    return LINES.reduce((total, line, index) => {
      ...
      const team2Score = calculateMemberScore(team2Members[index], line);

      return total + Math.abs(team1Score - team2Score);
    }, 0);
  };

  const getMaxLineDiffByMembers = (team1Members: Member[], team2Members: Member[]) => {
    return Math.max(
  ...
  const autoBalanceTeams = () => {
    const targetMembers = getAutoBalanceMembers();

    if (!targetMembers) {
      alert("10명의 모임원 이름을 중복 없이 모두 입력해야 합니다.");
      return;
    }

    let bestTeam1: Member[] | null = null;
    let bestTeam2: Member[] | null = null;
    let bestBalanceScore = Number.MAX_SAFE_INTEGER;

    const combinations = getCombinations(targetMembers, 5);

    combinations.forEach((team1BaseMembers) => {
      const team2BaseMembers = targetMembers.filter(
    ...
    if (!bestTeam1 || !bestTeam2) {
      alert("자동 팀짜기에 실패했습니다.");
      return;
    }

    setTeamSlots({
      team1: buildTeamSlotsFromMembers(bestTeam1),
      team2: buildTeamSlotsFromMembers(bestTeam2),
    });
  };
        ...
        <aside className="w-64 border-r border-slate-800 bg-[#08111f] p-6">
          ...
          <nav className="mt-10 space-y-3 text-sm">
            <button
              ...
              className={`w-full rounded-lg px-4 py-3 text-left ${activeMenu === "members"
            ...
            <button
              ...
              className={`w-full rounded-lg px-4 py-3 text-left ${activeMenu === "balance"
                ? "bg-blue-600 text-white"
                : "text-slate-300"
                }`}
            >
            ...
            <button
              ...
              className={`w-full rounded-lg px-4 py-3 text-left ${activeMenu === "record"
                ? "bg-blue-600 text-white"
                : "text-slate-300"
                }`}
            >
            ...
            <button
              ...
              className={`w-full rounded-lg px-4 py-3 text-left ${activeMenu === "aramBalance"
                ? "bg-blue-600 text-white"
                : "text-slate-300"
                }`}
            >
            ...
            <button
              ...
              className={`w-full rounded-lg px-4 py-3 text-left ${activeMenu === "aramRecord"
                ? "bg-blue-600 text-white"
                : "text-slate-300"
                }`}
            >
            ...
            <button
              ...
              className={`w-full rounded-lg px-4 py-3 text-left ${activeMenu === "stats"
                ? "bg-blue-600 text-white"
                : "text-slate-300"
                }`}
            >
        ...
        <section className="flex-1 p-8">
          {activeMenu === "members" && (
            <>
              ...
              <div
                className={`overflow-hidden transition-all duration-500 ${isAddFormOpen
                        ...
                        {MEMBER_GAME_TYPES.map((gameType) => {
                          ...
                          return (
                            <button
                              ...
                              className={`rounded-full border px-4 py-2 text-sm font-bold transition ${isChecked
                          ...
                          {canSetMemberRole ? (
                            <div className="flex items-center gap-1 rounded-full border border-slate-700 bg-[#07101f] p-1">
                              <button
                                ...
                                className={`rounded-full px-3 py-1 text-xs font-bold transition ${!member.member_role
                              ...
                              <button
                                ...
                                className={`rounded-full px-3 py-1 text-xs font-bold transition ${member.member_role === "모임장"
                              ...
                              <button
                                ...
                                className={`rounded-full px-3 py-1 text-xs font-bold transition ${member.member_role === "운영진"
                            ...
                            member.member_role && (
                              <span
                                className={`rounded-full border px-3 py-1 text-xs font-black shadow-sm ${member.member_role === "모임장"
                                ...
                                {MEMBER_GAME_TYPES.map((gameType) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`rounded-full border px-3 py-1 text-xs font-bold transition ${isChecked
                                        ? "border-blue-500 bg-blue-600 text-white"
                  ...
                  <div className="space-y-3">
                    {LINES.map((line) => {
                      ...
                      return (
                        <div
                          key={`team1-${line}`}
                          className={`rounded-xl border p-4 ${diff > 0
                        ...
                        >
                          ...
                          <input
                            list="member-list"
                            className={`w-full rounded-lg border bg-[#07101f] px-3 py-2 outline-none ${isUnknownMemberName(teamSlots.team1[line])
                  ...
                  {LINES.map((line) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p
                          className={`mt-1 text-sm font-bold ${diff === 0
                            ? "text-slate-300"
                            : diff > 0
                              ? "text-blue-400"
                              : "text-red-400"
                    ...
                    {LINES.map((line) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <input
                            list="member-list"
                            className={`w-full rounded-lg border bg-[#07101f] px-3 py-2 outline-none ${isUnknownMemberName(teamSlots.team2[line])
                              ? "border-red-500 text-red-300"
                              : "border-slate-700"
                              }`}
                            placeholder={`${line} 이름 입력`}
              ...
              <div className="mt-8 rounded-2xl border border-slate-800 bg-[#111c2e] p-5">
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <button
                          ...
                          className={`rounded-lg border px-2 py-2 ${lineResults[line] === "team1"
                        ...
                        <button
                          ...
                          className={`rounded-lg border px-2 py-2 ${lineResults[line] === "team2"
                ...
                <div className="mt-5 grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`rounded-xl border px-4 py-3 font-bold ${matchResult === "team1"
                      ? "border-blue-400 bg-blue-600 text-white"
                  ...
                  <button
                    ...
                    className={`rounded-xl border px-4 py-3 font-bold ${matchResult === "team2"
                      ? "border-red-400 bg-red-600 text-white"
                      : "border-slate-700 bg-[#07101f] text-slate-300"
                      }`}
                  >
            ...
            <div className="rounded-2xl border border-slate-800 bg-[#0b1424] p-6">
              <div className="flex items-center justify-between">
                ...
                <div className="rounded-xl border border-slate-700 bg-[#111c2e] px-5 py-3 text-right">
                  <p className="text-xs text-slate-400">밸런스 결과</p>
                  <p
                    className={`mt-1 text-lg font-bold ${getAramTeamTotalScore("team1") === getAramTeamTotalScore("team2")
                      ? "text-slate-200"
              ...
              <div className="mt-6 grid grid-cols-[1fr_90px_1fr] gap-4">
                    ...
                    {aramTeamSlots.team1.map((value, index) => {
                      ...
                      return (
                        <div
                          ...
                          className={`rounded-xl border p-4 ${isAce
                        ...
                        >
                          ...
                          <input
                            ...
                            className={`w-full rounded-lg border bg-[#07101f] px-3 py-2 outline-none ${isUnknownMemberName(value)
                              ? "border-red-500 text-red-300"
                              : "border-slate-700"
                              }`}
                            placeholder="이름 입력"
                          ...
                          <div className="mt-2 flex items-center justify-between gap-3">
                            ...
                            <button
                              ...
                              disabled={!member}
                              className={`shrink-0 rounded-lg border px-3 py-2 text-xs font-bold ${!member
                ...
                <div className="flex flex-col items-center justify-center gap-3">
                  ...
                  <div className="flex h-[120px] w-full flex-col items-center justify-center rounded-xl border border-slate-800 bg-[#111c2e]">
                    ...
                    <p
                      className={`mt-1 text-sm font-bold ${getAramTeamTotalScore("team1") === getAramTeamTotalScore("team2")
                        ? "text-slate-300"
                        : getAramTeamTotalScore("team1") >
                          getAramTeamTotalScore("team2")
                          ? "text-blue-400"
              ...
              <div className="mt-8 rounded-2xl border border-slate-800 bg-[#111c2e] p-5">
                ...
                <div className="mt-5 grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`rounded-xl border px-4 py-3 font-bold ${aramMatchResult === "team1"
                      ? "border-blue-400 bg-blue-600 text-white"
                      : "border-slate-700 bg-[#07101f] text-slate-300"
                      }`}
                  >
                  ...
                  <button
                    ...
                    className={`rounded-xl border px-4 py-3 font-bold ${aramMatchResult === "team2"
                      ? "border-red-400 bg-red-600 text-white"
                      : "border-slate-700 bg-[#07101f] text-slate-300"
                      }`}
                  >
            ...
            <div className="rounded-2xl border border-slate-800 bg-[#0b1424] p-6">
              ...
              <div className="mt-8 flex gap-3">
                <button
                  ...
                  className={`rounded-xl border px-5 py-3 font-bold ${statsView === "records"
                    ? "border-blue-500 bg-blue-600 text-white"
                    : "border-slate-700 bg-[#07101f] text-slate-300"
                    }`}
                >
                ...
                <button
                  ...
                  className={`rounded-xl border px-5 py-3 font-bold ${statsView === "aramRecords"
                ...
                <button
                  ...
                  className={`rounded-xl border px-5 py-3 font-bold ${statsView === "members"
                    ? "border-blue-500 bg-blue-600 text-white"
                    : "border-slate-700 bg-[#07101f] text-slate-300"
                    }`}
                >
                  ...
                  {matchRecords.map((record, index) => (
                    ...
                    >
                      <div className="mb-5 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          <div
                            className={`rounded-xl border px-5 py-3 text-right ${record.match_result === "team1"
                      ...
                      <div className="grid grid-cols-5 gap-3">
                        {LINES.map((line) => (
                          <div
                            key={`${record.id}-${line}`}
                            className={`rounded-xl border p-4 ${record.line_results[line] === "team1"
                              ? "border-blue-500 bg-blue-900/40 text-blue-200"
                  ...
                  {filteredMemberStats.map((stat) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`overflow-hidden transition-all duration-500 ${isExpanded
